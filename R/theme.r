...
scglrTheme <- function(formula, data, H, family, size = NULL, weights = NULL,
  ...
  if(Formula:::length.Formula(formula)[1] != 1)
  ...
  # check part counts
  if(Formula:::length.Formula(formula)[2] < 1+additional)
    if(additional) {
  ...
  # theme count
  theme_R <- Formula:::length.Formula(formula)[2] - additional
  ...
  if(additional) {
    theme_A <- stats::terms(formula, lhs=0, rhs=Formula:::length.Formula(formula)[[2]])
  } else {
    theme_A <- NULL
  }
