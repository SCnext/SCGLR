...
```{r screeplot, fig.alt="Inertia per components"}
screeplot(genus.scglr)
```

# Correlation plot for first two components

```{r corr_plots,fig.show="hold",out.width="50%", fig.alt=c("Correlation plot showing covariates", "Correlation plot showing predictors")}
...
```{r pairplot,fig.asp=2, fig.alt="Explore components pairwise"}
...
```{r theme_screeplot, fig.alt="Inertia per components for each theme"}
screeplot(testthm)
```

## Specialized correlation plot (one for each theme)

```{r theme_corr_plots, fig.alt="Correlation plots for each theme"}
plot(testthm)
```
